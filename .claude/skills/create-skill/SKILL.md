...
|----------|-------------|
| **`.claude/skills/<name>/`** | Default for this project: team-shared, versioned. |
...
Prefer **plugin `.claude/skills/`** for anything that should match how the rest of the team works.
...
- **Referencing other skills**. For human-readable links, give the path from the repo root (e.g. `.claude/skills/create-issue/SKILL.md`). From a sibling folder, a relative link works too: `[create-issue](../create-issue/SKILL.md)`. Parent skills should delegate steps instead of duplicating long procedures.
